...
const Blog = () => {
  ...
  useEffect(() => {
    fetch(`https://blog-backend-s667.onrender.com/${id}`).then((response) => {
  ...
  return (
    <div>
      ...
      <div className="blogDate">
        {format(new Date(blogInfo.createdAt), "dd-MM-yyyy, hh:mm a")}
      </div>
      <div className="blogAuthor">~{blogInfo.author.username}</div>
      {userInfo.id === blogInfo.author._id && newLocal}
      <img
        src={`https://blog-backend-s667.onrender.com/` + blogInfo.cover}
        alt="Blog"
      />
      <div
